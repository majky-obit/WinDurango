...
namespace d3d11x {
    HRESULT ID3D11ResourceWrapperX::QueryInterface(REFIID riid, void** ppvObject)
	{
		...
		DEBUGPRINT("[ID3D11ResourceWrapperX] QueryInterface: %s\n", iidstr);
    ...
    {
        DEBUGPRINT("[ID3D11ResourceWrapperX] --> AddRef\n");
        return InterlockedIncrement(&m_RefCount);
    }

    ULONG ID3D11ResourceWrapperX::Release( )
    {
        DEBUGPRINT("[ID3D11ResourceWrapperX] --> Release\n");
    ...
    {
        DEBUGPRINT("[ID3D11ResourceWrapperX]: SetName STUB\n");
        return S_OK;
    }
    ...
    {
        DEBUGPRINT("[ID3D11ResourceWrapperX]: GetDescriptor STUB\n");
    }
}