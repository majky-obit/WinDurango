...
struct XG_TEXTURE2D_DESC
{
    ...
    XG_USAGE Usage;
    uint32_t BindFlags;                 // Any of XG_BIND_FLAG.
    uint32_t CPUAccessFlags;            // Any of XG_CPU_ACCESS_FLAG.
    uint32_t MiscFlags;                 // Any of XG_RESOURCE_MISC_FLAG.
    uint32_t ESRAMOffsetBytes;
    uint32_t ESRAMUsageBytes;
...
struct XG_RESOURCE_LAYOUT
{
    ...
    unsigned long BaseAlignmentBytes;
    uint32_t MipLevels;
    uint32_t Planes;
...
struct XG_MIPLEVEL_LAYOUT
{
    ...
    unsigned long Slice2DSizeBytes;
    uint32_t PitchPixels;
    uint32_t PitchBytes;
    uint32_t AlignmentBytes;
    uint32_t PaddedWidthElements;
    uint32_t PaddedHeightElements;
    uint32_t PaddedDepthOrArraySize;
    uint32_t WidthElements;
    uint32_t HeightElements;
    uint32_t DepthOrArraySize;
    uint32_t SampleCount;
    XG_TILE_MODE TileMode;
    unsigned long BankRotationAddressBitMask;
    unsigned long BankRotationBytesPerSlice;
    uint32_t SliceDepthElements;
};


void DetileTexture(D3D11_TEXTURE2D_DESC* pDesc, ID3D11Texture2D** PlacementTexture, UINT TileModeIndex)
{
    for (UINT mips; mips < pDesc->MipLevels; mips++)
    {
